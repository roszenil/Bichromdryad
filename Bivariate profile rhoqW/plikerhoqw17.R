...
rhoqw.grid<-as.matrix(read.csv("grid17.csv",header=FALSE,sep=","))
long1<-dim(rhoqw.grid)[1]
results<-matrix(rep(0,9*long1),ncol=9)
...
log.x0<-log(c(0.12618457,0.001583056,0.249210759,0.002270911,0.036268349,0.040118644,1.792317852,1.57E-14))

my.options<-list("algorithm"="NLOPT_LN_SBPLX","ftol_rel"=1e-05,"print_level"=1,"maxtime"=86400,"maxeval"=500)
for(i in 1:long1){
	 a<-log(rhoqw.grid[i,])
	 mle.loop<-nloptr(x0=log.x0,eval_f=negloglikeprofile, opts=my.options, bichrom.phy=angiosperm.tree,bichrom.data=bichrom.dataset, max.chromosome=last.state, pi.0=p.0,log.prof0=a)
	 print(mle.loop)
	  results[i,1:8]<-exp(mle.loop$solution)
	  results[i,9]<-mle.loop$objective  
	 log.x0<-mle.loop$solution write.table(results,file="resultsrhoqw17.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
