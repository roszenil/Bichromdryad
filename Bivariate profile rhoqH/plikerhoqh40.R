...
rhoqh.grid<-as.matrix(read.csv("grid40.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh40.csv",header=FALSE,sep=","))
...
for(i in 19:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh40.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
