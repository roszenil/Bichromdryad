...
rhoqh.grid<-as.matrix(read.csv("grid30.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh30.csv",header=FALSE,sep=","))
...
for(i in 21:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh30.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
