...
rhoqh.grid<-as.matrix(read.csv("grid31.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh31.csv",header=FALSE,sep=","))
...
for(i in 21:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh31.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
