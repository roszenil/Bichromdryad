...
rhoqh.grid<-as.matrix(read.csv("grid24.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh24.csv",header=FALSE,sep=","))
...
for(i in 13:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh24.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
