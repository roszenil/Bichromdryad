...
rhoqh.grid<-as.matrix(read.csv("grid28.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh28.csv",header=FALSE,sep=","))
...
for(i in 24:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh28.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
