...
rhoqh.grid<-as.matrix(read.csv("grid17.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh17.csv",header=FALSE,sep=","))
...
for(i in 13:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh17.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
