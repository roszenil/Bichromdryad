...
rhoqh.grid<-as.matrix(read.csv("grid4.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh4.csv",header=FALSE,sep=","))
...
for(i in 10:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh4.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
