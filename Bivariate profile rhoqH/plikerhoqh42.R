...
rhoqh.grid<-as.matrix(read.csv("grid42.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh42.csv",header=FALSE,sep=","))
...
for(i in 18:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh42.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
