...
rhoqh.grid<-as.matrix(read.csv("grid25.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh25.csv",header=FALSE,sep=","))
...
for(i in 11:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh25.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
