...
rhoqh.grid<-as.matrix(read.csv("grid44.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh44.csv",header=FALSE,sep=","))
...
for(i in 11:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh44.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
