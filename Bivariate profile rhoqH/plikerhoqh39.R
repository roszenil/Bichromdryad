...
rhoqh.grid<-as.matrix(read.csv("grid39.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh39.csv",header=FALSE,sep=","))
...
for(i in 17:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh39.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
