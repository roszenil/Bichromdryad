...
rhoqh.grid<-as.matrix(read.csv("grid41.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh41.csv",header=FALSE,sep=","))
...
for(i in 20:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh41.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
