...
rhoqh.grid<-as.matrix(read.csv("grid3.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh3.csv",header=FALSE,sep=","))
...
for(i in 11:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh3.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
