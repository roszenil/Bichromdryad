...
rhoqh.grid<-as.matrix(read.csv("grid9.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh9.csv",header=FALSE,sep=","))
...
for(i in 11:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh9.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
