...
rhoqh.grid<-as.matrix(read.csv("grid37.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh37.csv",header=FALSE,sep=","))
...
for(i in 19:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh37.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
