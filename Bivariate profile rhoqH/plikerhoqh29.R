...
rhoqh.grid<-as.matrix(read.csv("grid29.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh29.csv",header=FALSE,sep=","))
...
for(i in 22:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh29.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
