...
rhoqh.grid<-as.matrix(read.csv("grid36.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh36.csv",header=FALSE,sep=","))
...
for(i in 18:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh36.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
