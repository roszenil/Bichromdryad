...
rhoqh.grid<-as.matrix(read.csv("grid35.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh35.csv",header=FALSE,sep=","))
...
for(i in 20:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh35.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
