...
rhoqh.grid<-as.matrix(read.csv("grid43.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh43.csv",header=FALSE,sep=","))
...
for(i in 12:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh43.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
