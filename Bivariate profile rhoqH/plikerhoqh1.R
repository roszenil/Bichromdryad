...
rhoqh.grid<-as.matrix(read.csv("grid1.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh1.csv",header=FALSE,sep=","))
...
for(i in 23:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh1.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
