...
rhoqh.grid<-as.matrix(read.csv("grid26.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh26.csv",header=FALSE,sep=","))
...
for(i in 22:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh26.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
