...
rhoqh.grid<-as.matrix(read.csv("grid14.csv",header=FALSE,sep=","))
long1<-dim(rhoqh.grid)[1]
results<-as.matrix(read.csv("resultsrhoqh14.csv",header=FALSE,sep=","))
...
for(i in 11:long1){
	 ...
	 print(mle.loop)
	  ...
	  write.table(results,file="resultsrhoqh14.csv",sep=",",col.names=FALSE,row.names=FALSE)
 }
